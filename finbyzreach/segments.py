...
def _calculate_static_leads(filter_groups_json, exclude_filters_json, exclude_email_groups_json):
	...
	return sorted(list(include_leads - exclude_leads))
...
def create_segment(segment_name, segment_type, filter_groups, exclude_filters="[]", exclude_email_groups="[]"):
	...
	if segment_type == "Static":
		member_names = _calculate_static_leads(
			filter_groups_json, exclude_filters_json, exclude_email_groups_json
		)
...
@frappe.whitelist(methods=["POST"])
def update_segment(segment_name, filter_groups, exclude_filters="[]", exclude_email_groups="[]"):
	...
	doc.exclude_email_groups_json = exclude_email_groups
	doc.save()
...
def refresh_static_segment_leads(segment_name):
	...
	doc.static_leads_json = json.dumps(member_names, separators=(",", ":"))
	doc.member_count = len(member_names)
	doc.save()
